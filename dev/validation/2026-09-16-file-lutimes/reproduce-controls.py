"""Check link timestamp regressions against isolated runtime defects."""
import argparse
import hashlib
import json
...
operation = '''    case 47: {
      ...
      await lutimes(args[0], atime, mtime);
      return Buffer.alloc(0);
    }
'''
call = 'await lutimes(args[0], atime, mtime);'
variants = [
    ('positive', None, None),
    ('missing-operation', operation, ''),
    ('missing-arity', ', 3, 3, 3, 3];', ', 3, 3, 3];'),
    ('follow-target', call, 'await utimes(args[0], atime, mtime);'),
    ('swap-times', call, 'await lutimes(args[0], mtime, atime);'),
    ('normalize-path', call, 'await lutimes(resolve(args[0]), atime, mtime);'),
    ('update-before-validation', operation, operation.replace(
        'const mtime = timestamp(args[2]);',
        'await lutimes(args[0], atime, new Date(0)); const mtime = timestamp(args[2]);')),
    ('negative-seconds', call, 'await lutimes(args[0], atime.getTime() / 1000, mtime.getTime() / 1000);'),
    ('noncanonical-times', 'String(milliseconds) !== value', 'false'),
]
results = []
for name, old, new in variants:
    ...
    with tempfile.TemporaryDirectory(prefix='veil-lutimes-control-') as directory:
        work = Path(directory)
        (work / 'runtime').mkdir()
        (work / 'dev').mkdir()
        (work / 'runtime/reactor.mjs').write_text(changed)
        (work / 'dev/runtime-test.mjs').write_bytes(tests)
        command = ['node', '--test', '--test-reporter=tap', '--test-name-pattern=^file lutimes ',
