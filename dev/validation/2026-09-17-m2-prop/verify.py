#!/usr/bin/env python3
"""Verify the closed proposition translation record and its source bindings."""
...
WORKSPACE = "/Users/oobi/Documents/gpt3/veil-m2-prop"
...
COMMANDS = {
    ...
    "record": (["python3", "-I", "dev/m2-translate.py", "record", "--output",
                "dev/m2-translation-prop-candidate"], 0),
...
def verify():
    ...
    require(set(results) == {"base", "workspace", "checks", "sources"}, "unexpected record schema")
    ...
    require(sample["checker_sha256"] == CHECKER_SHA256, "reused checker fingerprint changed")
    for label in ("record", "live"):
        require(json.loads((RECORD / f"{label}.stdout").read_text()) == summary, f"{label} summary changed")
    for label, total, ending in (("translation-tests", 33, "OK\n"),
    ...
    return "Closed proposition validation: seven captures, source hashes and five sample rechecks verified."
...
if __name__ == "__main__":
    try:
        ...
        print(f"Closed proposition validation: {error}", file=sys.stderr)
        sys.exit(1)
