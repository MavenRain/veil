"""Run deliberate defects in temporary copies; never modify recorded captures."""
...
def main():
    ...
    arm = 'case 34: return Buffer.from(String((await stat(args[0], { bigint: true })).atimeNs));'
    arities = next(line for line in source.splitlines() if line.startswith('const requestArities ='))
    variants = [
        ('base-runtime', baseline, '^file accessed', 'Runtime from the base commit lacks operation 34.'),
        ('wrong-timestamp', replace_once(source, arm, arm.replace('.atimeNs', '.mtimeNs')),
         '^file accessed', 'Return modification time instead of access time.'),
        ('rounded-number', replace_once(source, arm,
         'case 34: return Buffer.from(String(Number((await stat(args[0], { bigint: true })).atimeNs)));'),
         '^file accessed', 'Round the access time through Number before formatting.'),
        ('inspect-symlink', replace_once(source, arm, arm.replace('await stat(', 'await lstat(')),
         '^file accessed', 'Inspect the final symlink instead of following it.'),
        ('omit-arity', replace_once(source, arities, arities.removesuffix(', 1];') + '];'),
         '^file accessed', 'Remove the arity entry for operation 34.'),
        ('normalize-path', replace_once(source, arm, arm.replace('stat(args[0],', 'stat(resolve(args[0]),')),
         '^file accessed', 'Lexically normalize paths before native symlink resolution.'),
        ('open-contents', replace_once(source, arm, "case 34: await open(args[0], 'r'); " + arm.removeprefix('case 34: ')),
         '^file accessed preserves exact', 'Open the requested entry before reading its metadata.'),
        ('modified-read-contents', replace_once(source, 'case 33: return Buffer.from',
         'case 33: await readFile(args[0]); return Buffer.from'),
         '^file modified preserves exact', 'Read the requested entry before returning modification time.'),
    ]
    assert not args.output.exists(), 'Use a fresh output directory'
    args.output.mkdir(parents=True)
    controls = []
    with tempfile.TemporaryDirectory(prefix='veil-accessed-controls-') as temporary:
