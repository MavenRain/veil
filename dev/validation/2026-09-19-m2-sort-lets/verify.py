#!/usr/bin/env python3
"""Verify the closed-sort let record and its source bindings."""
...
WORKSPACE = "/Users/oobi/Documents/gpt3/veil-m2-sort-lets"
...
COMMANDS = {
    ...
    "record": (["python3", "-I", "dev/m2-translate.py", "record", "--output",
                ".kanon-exec/m2-translation-sort-lets-candidate"], 0),
    ...
    "compiler-reuse": (["python3", "-I", "dev/validation/2026-09-19-m2-sort-lets/verify.py",
                        "--compiler-reuse"], 0),
    "controls": (["python3", "-I", "dev/validation/2026-09-19-m2-sort-lets/controls.py"], 0),
}
...
def verify():
    ...
    for label, total, ending in (("translation-tests", 79, "OK\n"),
    ...
    require(controls == {"baseline_passed": True, "no_local_lets_killed": True,
                         "discard_scope_killed": True}, "mutation verdicts changed")
    ...
    return "Closed-sort let validation: nine captures, source hashes and five sample rechecks verified."
...
if __name__ == "__main__":
    try:
        ...
        print(f"Closed-sort let validation: {error}", file=sys.stderr)
        sys.exit(1)
