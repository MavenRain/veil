...
The JavaScript reactor validates the argument count of operations 1 to 49
...
[file append regressions](FILE-APPEND.md) record row 29, the
[file truncate regressions](FILE-TRUNCATE.md) record row 30, the
[file mode regressions](FILE-MODE.md) record row 31, the
[permission inspection regressions](FILE-PERMISSIONS.md) record row 32, the
[modification time regressions](FILE-MODIFIED.md) record row 33, the
[access time regressions](FILE-ACCESSED.md) record row 34, the
[status-change time regressions](FILE-CHANGED.md) record row 35, the
[creation time regressions](FILE-CREATED.md) record row 36, the
[file identity regressions](FILE-IDENTITY.md) record row 37, the
[link count regressions](FILE-LINK-COUNT.md) record row 38, the
[file owner regressions](FILE-OWNER.md) record row 39, the
[file allocation regressions](FILE-ALLOCATION.md) record row 40, the
[filesystem capacity regressions](FILESYSTEM-CAPACITY.md) record row 41, the
...
[link ownership regressions](FILE-LCHOWN.md) record row 46, the
[link timestamp regressions](FILE-LUTIMES.md) record row 47, the
[file access regressions](FILE-ACCESS.md) record row 48, and the
[file creation regressions](FILE-CREATE.md) record row 49.
...
Operation 48 requires exactly two arguments: path and a single mode digit
from 0 to 7.
Operation 49 requires exactly one path argument; its content is the body.
